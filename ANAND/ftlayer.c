...
/* iamos */
o_4b allocate_s_block();
o_4b allocate_ss_block();
o_4b my_merge(o_4b lsectornum);
...
void ftl_open()
{
	...
	{
		ramtable.L2P[i] = -1;
		ramtable.block_state[i] = (byte)STATE_F;
		/* iamos */
		for (j = 0; j < PAGES_PER_BLOCK; j++) {
			ramtable.page_state[i + j] = NEW_PAGE;
		}
	...
	for (i = 0; i < SPAREBLOCKS_PER_DEVICE / 2; i++) {
		ramtable.P2S[i] = -1;
		ramtable.S2S[i] = -1;
	}
...
void ftl_write(byte *sectorbuf, o_4b lsectornum)
{
	...
	if (pblock_num < 0)	/* a logical block is first allocated a physical block */
	{
		...
		/* iamos */
		ramtable.page_state[pagenum] = USED_PAGE;
		...
		pagenum = get_physicalpage_num(pblock_num, lsectornum);
		if ( ramtable.page_state[pagenum] == USED_PAGE) {
			int sblock_num = ramtable.P2S[pblock_num];
			if ( sblock_num < 0 ) {
				sblock_num = allocate_s_block();
				pagenum = get_physicalpage_num(sblock_num, lsectornum);

				ramtable.L2P[lblock_num] = sblock_num;
				ramtable.P2S[pblock_num] = sblock_num;
				ramtable.block_state[sblock_num] = (byte)STATE_U;
			}
			else {
				pagenum = get_physicalpage_num(sblock_num, lsectornum);
				if ( ramtable.page_state[pagenum] == USED_PAGE) {
					int ssblock_num = ramtable.S2S[sblock_num];
					if ( ssblock_num < 0) {
						ssblock_num = allocate_ss_block();
						pagenum = get_physicalpage_num(sblock_num, lsectornum);

						ramtable.L2P[lblock_num] = ssblock_num;
						ramtable.S2S[sblock_num] = ssblock_num;
						ramtable.block_state[ssblock_num] = (byte)STATE_U;
					}
					else {
						pagenum = get_physicalpage_num(ssblock_num, lsectornum);
						if ( ramtable.page_state[pagenum] == USED_PAGE) {
							// Merge
							int new_page_num = my_merge(lsectornum);
							pagenum = get_physicalpage_num(new_page_num, lsectornum);
						}
					}
				}
			}
...
o_4b my_merge(o_4b lsectornum) {
	o_4b lblock_num, pblock_num, sblock_num, ssblock_num, new_pblock_num, src_pagenum, dest_pagenum;
	int i;
	int valid_page_address[PAGES_PER_BLOCK];

	byte* pagebuf = (byte*)malloc(PAGE_SIZE);

	lblock_num = lsectornum / PAGES_PER_BLOCK;
	pblock_num = ramtable.L2P[lblock_num];
	sblock_num = ramtable.P2S[pblock_num];
	ssblock_num = ramtable.S2S[sblock_num];

	new_pblock_num = allocate_f_block();

	for ( i = 0; i < PAGES_PER_BLOCK ; i++) {
		valid_page_address[i] = -1;
	}

	// Merge from pblock
	for (i = 0; i < PAGES_PER_BLOCK; i++) {
		if ( ramtable.page_state[pblock_num + i] == USED_PAGE)
			valid_page_address[i] = pblock_num + i;
	}

	// Merge from sblock
	for (i = 0; i < PAGES_PER_BLOCK; i++) {
		if (ramtable.page_state[sblock_num + i] == USED_PAGE)
			valid_page_address[i] = sblock_num + i;
	}
	// Merge from ssblock
	for (i = 0; i < PAGES_PER_BLOCK; i++) {
		if (ramtable.page_state[ssblock_num + i] == USED_PAGE)
			valid_page_address[i] = ssblock_num + i;
	}
	// Cut the link of p, s, ss blocks
	ramtable.L2P[lsectornum] = new_pblock_num;
	ramtable.P2S[pblock_num] = -1;
	ramtable.S2S[sblock_num] = -1;
	ramtable.block_state[new_pblock_num] = (byte)STATE_U;

	// COPY to NEW_PBLOCK
	for ( i = 0; i < PAGES_PER_BLOCK; i++) {
		if (valid_page_address[i] > 0) {
			src_pagenum = valid_page_address[i];
			read(pagebuf, src_pagenum, MODE_FILE);
			write(pagebuf, new_pblock_num + i, MODE_FILE);
		}
	}
	free(pagebuf);
...
o_4b allocate_s_block()
{
	...
	// assert(first_s_block_idx >= 0);

	return first_s_block_idx;
}

o_4b allocate_ss_block()
{
	o_4b first_ss_block_idx;
	int i;

	first_ss_block_idx = -1;
	...
	for (i = 3000; i < 4500; i++)
	{
		...
		if (ramtable.block_state[i] == (byte)STATE_F)
		{
			first_ss_block_idx = i;
			break;
		}
	}

	// assert(first_ss_block_idx >= 0);

	return first_ss_block_idx;
}
/*
void erase_block(o_4b pblock_num)
{
